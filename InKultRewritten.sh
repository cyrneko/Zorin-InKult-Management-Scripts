...
apt-fast-fallback() {
	if [ "$(which apt-fast)" != "/usr/bin/apt-fast" ]; then
		alias "apt-fast"="apt"
	fi
}
...
basic-programs() {
	...
	echo -e "${green}${bold}# Installing WINE keys...${reset}"
	wget -nc https://dl.winehq.org/wine-builds/winehq.key
	sudo mv winehq.key /usr/share/keyrings/winehq-archive.key
	echo -e "${green}${bold}# Installing WINE repositories...${reset}"
	wget -nc https://dl.winehq.org/wine-builds/ubuntu/dists/focal/winehq-focal.sources
	sudo mv winehq-focal.sources /etc/apt/sources.list.d/
	echo -e "${green}${bold}# Installing WINE...${reset}"
	sudo apt-fast install --install-recommends winehq-devel
	echo -e "${green}${bold}# Installation and setup complete :)${reset}"
}
